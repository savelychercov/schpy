...
plt.plot(x, y1, linestyle='-', color='blue', label='Линия 1')   # Сплошная линия
plt.plot(x, y2, linestyle='--', color='red', label='Линия 2')   # Пунктирная линия
plt.plot(x, y3, linestyle=':', color='green', label='Линия 3')   # Штриховая линия

# Настройка графика
plt.title('Несколько линий с разными стилями')
...
if __name__ == "__main__":
    ...
    for seed in range(1, count_iterations+1):
        if time.time() - (passed_time + start_time) > update_every:  # condition: every 1 second
            ...
            best_schedule_counts = best_of.get_counts(best_schedule_obj.pairs, best_data,
                                                      best_schedule_obj.remaining_data)
        ...
        schedule_rating = best_of.rate_schedule(schedule_obj.pairs, data_copy, schedule_obj.remaining_data)
        ...
        best_schedule_counts = best_of.get_counts(best_schedule_obj.pairs, best_data, best_schedule_obj.remaining_data)
        plot_data.append((
            seed,
            best_rating,
    ...
    print(f"Teachers gaps: {best_of.count_teachers_gaps(best_data, best_schedule_obj.remaining_data)}")
    print(f"Offline pairs gaps: {best_of.count_offline_pairs_gaps(best_schedule_obj.pairs, best_data)}")
    print(f"Overworked teachers: {best_of.count_overworked_teachers(best_schedule_obj.pairs)}")
    print(f"Unissued hours: {best_of.count_unissued_hours(best_schedule_obj.remaining_data)}")

    plt.xlabel('Iterations')
    plt.plot(range(1, count_iterations+1), [x[1] for x in plot_data], linestyle='-', color='blue', label='Rating')
    plt.text(
        count_iterations+1,
    ...
    plt.plot(range(1, count_iterations+1), [x[2] for x in plot_data], linestyle='-', color='red', label='Teachers gaps')
    plt.text(
        count_iterations + 1,
        [x[2] for x in plot_data][-1],
        "Teachers gaps",
        fontsize=10,
        ha='left',
        color="white",
    ...
    plt.plot(range(1, count_iterations+1), [x[3] for x in plot_data], linestyle='-', color='green', label='Offline pairs gaps')
    plt.text(
        count_iterations + 1,
    ...
    plt.plot(range(1, count_iterations+1), [x[4] for x in plot_data], linestyle='-', color='orange', label='Overworked teachers')
    plt.text(
        count_iterations + 1,
        [x[4] for x in plot_data][-1],
        "Overworked teachers",
        fontsize=10,
        ha='left',
        color="white",
    ...
    plt.plot(range(1, count_iterations+1), [x[5] for x in plot_data], linestyle='-', color='purple', label='Unissued hours')
    plt.text(
        count_iterations + 1,
